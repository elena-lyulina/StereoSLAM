...
static void BM_FeatureMatching(benchmark::State& state) {
  for (auto _ : state) {
    ...
    frame.suppression2D (loc, frame.getBlobConvolution (result), blobMaxCoord, 1);
    vector<FeaturePoint> blobMaxPoints;
    for (auto p : blobMaxCoord) {
      blobMaxPoints.emplace_back(FeaturePoint(frame, p.first, p.second));
    }

    vector<pair<FeaturePoint, FeaturePoint>> matchesBMax;


    FeatureDetection::findMatches (blobMaxPoints, blobMaxPoints, matchesBMax, error);
  }
}
