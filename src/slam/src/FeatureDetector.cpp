...
: predFrames (&predFrames), succFrames (&succFrames)
{
    ...
    {
        frames[i]->suppression2D (supprArea, frames[i]->getBlobConvolution (), coord[i][BLOB_MAX],
                                  coord[i][BLOB_MIN]);
        frames[i]->suppression2D (supprArea, frames[i]->getCornerConvolution (),
                                  coord[i][CORNER_MAX], coord[i][CORNER_MIN]);
