...
int main (int argc, char *argv[])
{
    ...
    for (int i = 0; i < filenames.size (); i++)
    {
        ...
        frames[i].suppression2D (n, frames[i].getBlobConvolution (results[i]), blobMaxCoord[i],
                                 greater<int> ());
        frames[i].suppression2D (n, frames[i].getBlobConvolution (results[i]), blobMinCoord[i], less<int> ());
        frames[i].suppression2D (n, frames[i].getCornerConvolution (results[i]), cornerMaxCoord[i],
                                 greater<int> ());
        frames[i].suppression2D (n, frames[i].getCornerConvolution (results[i]), cornerMinCoord[i],
                                 less<int> ());
    ...
    FeatureDetection::doMatchingCircle (blobMaxPoints[0], blobMaxPoints[1], blobMaxPoints[2],
                                        blobMaxPoints[3], matchesBMax);
    ...
    FeatureDetection::doMatchingCircle (blobMinPoints[0], blobMinPoints[1], blobMinPoints[2],
                                        blobMinPoints[3], matchesBMin);
    cout << "matchBMin size " << matchesBMin.size () << endl;

    FeatureDetection::doMatchingCircle (cornerMaxPoints[0], cornerMaxPoints[1], cornerMaxPoints[2],
                                        cornerMaxPoints[3], matchesCMax);
    cout << "matchCMax size " << matchesCMax.size () << endl;

    FeatureDetection::doMatchingCircle (cornerMinPoints[0], cornerMinPoints[1], cornerMinPoints[2],
                                        cornerMinPoints[3], matchesCMin);
